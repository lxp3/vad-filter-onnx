...
LinearResample::LinearResample(int32_t samp_rate_in_hz, int32_t samp_rate_out_hz,
    ...
    assert(samp_rate_in_hz > 0.0 && samp_rate_out_hz > 0.0 && filter_cutoff_hz > 0.0 &&
           filter_cutoff_hz * 2 <= samp_rate_in_hz && filter_cutoff_hz * 2 <= samp_rate_out_hz &&
           num_zeros > 0);
...
void LinearResample::Resample(const float *input, int32_t input_dim, bool flush,
    ...
    for (int64_t samp_out = output_sample_offset_; samp_out < tot_output_samp; samp_out++) {
        ...
        // for.
        int32_t first_input_index = static_cast<int32_t>(first_samp_in - input_sample_offset_);
        float this_output = 0;
        if (first_input_index >= 0 &&
            first_input_index + static_cast<int32_t>(weights.size()) <= input_dim) {
            this_output = DotProduct(input + first_input_index, weights.data(), weights.size());
        ...
        int32_t output_index = static_cast<int32_t>(samp_out - output_sample_offset_);
...
void LinearResample::GetIndexes(int64_t samp_out, int64_t *first_samp_in,
    ...
    *samp_out_wrapped = static_cast<int32_t>(samp_out - unit_index * output_samples_in_unit_);
...
void LinearResample::SetRemainder(const float *input, int32_t input_dim) {
    ...
    int32_t max_remainder_needed = std::ceil(samp_rate_in_ * num_zeros_ / filter_cutoff_);
    input_remainder_.resize(max_remainder_needed);
    for (int32_t index = -static_cast<int32_t>(input_remainder_.size()); index < 0; index++) {
        ...
        if (input_index >= 0) {
            ...
            input_remainder_[index + static_cast<int32_t>(input_remainder_.size())] =
                old_remainder[input_index + static_cast<int32_t>(old_remainder.size())];
