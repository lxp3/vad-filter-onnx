...
DeepFilterNetDenoiseModel::DeepFilterNetDenoiseModel(const DeepFilterNetDenoiseModel &other,
...
std::unique_ptr<DenoiseModel> DeepFilterNetDenoiseModel::init(const DenoiseConfig &config) {
    if (config.sample_rate != sample_rate_) {
        throw std::invalid_argument("DeepFilterNet model only supports a " +
                                    std::to_string(sample_rate_) + " Hz sample rate");
    }
    return std::unique_ptr<DenoiseModel>(new DeepFilterNetDenoiseModel(*this, config));
}

void DeepFilterNetDenoiseModel::reset() {
...
std::vector<float> DeepFilterNetDenoiseModel::forward(const float *speech) {
...
void DeepFilterNetDenoiseModel::compact_input() {
    if (input_offset_ == 0) {
        return;
    }
...
std::vector<float> DeepFilterNetDenoiseModel::decode(const float *data, int n,
                                                     bool input_finished) {
