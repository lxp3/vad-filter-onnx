...
int main(int argc, char **argv) {
    try {
        const std::string model_path = argc > 1 ? argv[1] : "public/models/deepfilternet2.onnx";
        const int sample_rate = argc > 2 ? std::stoi(argv[2]) : 48000;
        auto handle = AutoDenoiseModel::create(model_path);
        if (!handle) {
            throw std::runtime_error("Failed to create DeepFilterNet model");
        }
        DenoiseConfig config;
        config.sample_rate = sample_rate;
        auto model = handle->init(config);
        if (!model) {
            throw std::runtime_error("Failed to initialize DeepFilterNet model");
        }
        ...
        if (expected.size() != samples.size()) {
            throw std::runtime_error("DeepFilterNet output is not input-length aligned");
        }
        for (int chunk_size : { 1, 100, 480, 500 }) {
            CheckEqual(expected, Decode(model.get(), samples, chunk_size));
        }

        for (int length : { 0, 1, 479, 480, 481 }) {
            ...
            if (output.size() != short_input.size()) {
                throw std::runtime_error("Short DeepFilterNet output length mismatch");
            }
        }

        CheckEqual(expected, Decode(model.get(), samples, 480));
        ...
        try {
            ...
            throw std::runtime_error("DeepFilterNet accepted an unsupported sample rate");
        } catch (const std::invalid_argument &) {
        }
        std::cout << "DeepFilterNet denoise streaming checks passed; samples=" << expected.size()
                  << '\n';
        return 0;
