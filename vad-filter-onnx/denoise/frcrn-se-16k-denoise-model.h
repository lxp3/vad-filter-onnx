...
// I/O and its STFT/iSTFT operate over the whole utterance, so it cannot be
// run incrementally. decode() buffers every sample it is given and never
// runs the ONNX session until input_finished == true; no output is ever
// returned before the stream finishes.
