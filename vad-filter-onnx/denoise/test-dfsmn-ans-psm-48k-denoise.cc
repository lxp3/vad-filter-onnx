...
int main(int argc, char **argv) {
    try {
        const std::string model_path = argc > 1 ? argv[1] : "public/models/dfsmn_ans_psm_48k.onnx";
        const int sample_rate = argc > 2 ? std::stoi(argv[2]) : 48000;
        auto handle = AutoDenoiseModel::create(model_path);
        if (!handle) {
            throw std::runtime_error("Failed to create DFSMN-ANS-PSM 48k model");
        }
        DenoiseConfig config;
        config.sample_rate = sample_rate;
        auto model = handle->init(config);
        if (!model) {
            throw std::runtime_error("Failed to initialize DFSMN-ANS-PSM 48k model");
        }
        ...
        if (expected.size() != samples.size()) {
            throw std::runtime_error("DFSMN-ANS-PSM 48k output is not input-length aligned");
        }
        for (int chunk_size : { 1, 100, 960, 2000 }) {
            CheckEqual(expected, Decode(model.get(), samples, chunk_size));
        }

        for (int length : { 0, 1, 959, 960, 961 }) {
            ...
            if (output.size() != short_input.size()) {
                throw std::runtime_error("Short DFSMN-ANS-PSM 48k output length mismatch");
            }
        }

        CheckEqual(expected, Decode(model.get(), samples, 960));
        ...
        try {
            ...
            throw std::runtime_error("DFSMN-ANS-PSM 48k accepted an unsupported sample rate");
        } catch (const std::invalid_argument &) {
        }
        std::cout << "DFSMN-ANS-PSM 48k denoise streaming checks passed; samples="
                  << expected.size() << '\n';
        return 0;
