...
int main(int argc, char **argv) {
    try {
        const std::string model_path = argc > 1 ? argv[1] : "public/models/frcrn_se_16k.onnx";
        const int sample_rate = argc > 2 ? std::stoi(argv[2]) : 16000;
        auto handle = AutoDenoiseModel::create(model_path);
        if (!handle) {
            throw std::runtime_error("Failed to create FRCRN model");
        }
        DenoiseConfig config;
        config.sample_rate = sample_rate;
        auto model = handle->init(config);
        if (!model) {
            throw std::runtime_error("Failed to initialize FRCRN model");
        }
        ...
        for (std::size_t offset = 0; offset < samples.size(); offset += kChunkSize) {
            ...
            if (!finished && !output.empty()) {
                throw std::runtime_error("FRCRN returned output before input_finished");
            }
            chunked.insert(chunked.end(), output.begin(), output.end());
        }
        if (chunked.size() != whole.size()) {
            throw std::runtime_error("Chunked-input FRCRN output length mismatch");
        }
        float max_diff = 0.0F;
        ...
        if (max_diff > 1e-6F) {
            throw std::runtime_error("Chunked input feeding changed FRCRN output");
        }
        ...
        if (!threw) {
            throw std::runtime_error("FRCRN accepted decode() after stream finished");
        }
        ...
        try {
            ...
            throw std::runtime_error("FRCRN accepted an unsupported sample rate");
        } catch (const std::invalid_argument &) {
        }

        std::cout << "FRCRN denoise offline checks passed; samples=" << whole.size() << '\n';
        return 0;
