...
bool is_deepfilternet_denoise(const std::vector<const char *> &input_names,
                              const std::vector<const char *> &output_names);

// DeepFilterNet (v1/v2/v3) real-time streaming denoise model. All three
// versions share an identical ONNX I/O contract (speech/analysis_cache/
// synthesis_cache/state_in -> enhanced/analysis_cache_out/
// synthesis_cache_out/state_out, matching DpdfnetDenoiseModel's convention),
// so a single class serves all of them; only `state_size_`/`hop_size_`/
// `sample_rate_` differ, and those are read from ONNX metadata.
class DeepFilterNetDenoiseModel : public DenoiseModel {
  public:
    DeepFilterNetDenoiseModel() = default;
    ...
    // Number of hops of pure algorithmic delay the model introduces (STFT
    // frame/hop mismatch plus any `conv_lookahead`), i.e. how many leading
    // hops of model output must be dropped and how many trailing zero-hops
    // must be flushed through at `input_finished` to keep output sample
    // count aligned with input sample count.
    void set_delay_hops(std::size_t delay_hops) { delay_hops_ = delay_hops; }

  private:
    DeepFilterNetDenoiseModel(const DeepFilterNetDenoiseModel &other, const DenoiseConfig &config);
