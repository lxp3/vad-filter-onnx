...
int main(int argc, char **argv) {
    try {
        const std::string model_path = argc > 1 ? argv[1] : "public/models/gtcrn.onnx";
        auto handle = AutoDenoiseModel::create(model_path);
        if (!handle) {
            throw std::runtime_error("Failed to create GTCRN model");
        }
        auto model = handle->init(DenoiseConfig{});
        if (!model) {
            throw std::runtime_error("Failed to initialize GTCRN model");
        }

        std::mt19937 generator(20260723);
        std::uniform_real_distribution<float> distribution(-1.0F, 1.0F);
        std::vector<float> samples(16000 + 37);
        ...
        if (expected.size() != samples.size()) {
            throw std::runtime_error("GTCRN output is not input-length aligned");
        }
        for (int chunk_size : { 1, 100, 256, 500 }) {
            CheckEqual(expected, Decode(model.get(), samples, chunk_size));
        }

        for (int length : { 0, 1, 255, 256, 257 }) {
            ...
            if (output.size() != short_input.size()) {
                throw std::runtime_error("Short GTCRN output length mismatch");
            }
        }

        CheckEqual(expected, Decode(model.get(), samples, 256));
        ...
        try {
            ...
            throw std::runtime_error("GTCRN accepted an unsupported sample rate");
        } catch (const std::invalid_argument &) {
        }
        std::cout << "GTCRN denoise streaming checks passed; samples=" << expected.size() << '\n';
        return 0;
