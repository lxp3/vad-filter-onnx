...
// Matches ClearerVoice-Studio's MossFormerGAN_SE_16K inference config:
// one_time_decode_length=10 (seconds), decode_window=10 (seconds).
constexpr double kDecodeWindowSeconds = 10.0;
...
MossformerganSe16kDenoiseModel::MossformerganSe16kDenoiseModel(
    const MossformerganSe16kDenoiseModel &other, const DenoiseConfig &config)
...
std::unique_ptr<DenoiseModel> MossformerganSe16kDenoiseModel::init(const DenoiseConfig &config) {
    if (config.sample_rate != sample_rate_) {
        throw std::invalid_argument("MossFormerGAN_SE_16K model only supports a " +
                                    std::to_string(sample_rate_) + " Hz sample rate");
    }
    return std::unique_ptr<DenoiseModel>(new MossformerganSe16kDenoiseModel(*this, config));
}

void MossformerganSe16kDenoiseModel::reset() {
    input_buffer_.clear();
    finished_ = false;
}

std::vector<float> MossformerganSe16kDenoiseModel::forward(const float *data, std::size_t n) {
...
std::vector<float> MossformerganSe16kDenoiseModel::decode_segmented() {
    const std::size_t t = input_buffer_.size();
    const std::size_t window =
        static_cast<std::size_t>(sample_rate_ * kDecodeWindowSeconds);
    if (t <= window) {
...
std::vector<float> MossformerganSe16kDenoiseModel::decode(const float *data, int n,
                                                           bool input_finished) {
